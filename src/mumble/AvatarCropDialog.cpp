...
AvatarCropDialog::AvatarCropDialog(const QImage &source, QWidget *parent) : QDialog(parent) {
	...
	m_zoomSlider->setToolTip(tr("Zoom: drag the image to move the crop area"));
...
void AvatarCropDialog::updateHint() {
	...
	const qreal zoom = effectiveZoom();
	m_hint->setText(tr("Output 512x512  ·  source %1x%2  ·  zoom %3%")
						.arg(m_source.width())
						.arg(m_source.height())
						.arg(qRound(zoom * 100)));
	m_hint->setToolTip(tr("Drag inside the square to move the image, use the slider to zoom. "
